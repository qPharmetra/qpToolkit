...
#' @description Plots residual, predicted, or partial residial plots based on input generated by qP functions nlme.predict and nls.predict. 
#' @param input the output from \code{nls.predict} or \code{nlme.predict}
#' @param newFunc the function to plot. If not suppliued will be taken from \code{input}
...
#' plot.fit(fit.PD004.pred.nlme) ## 'simple' prediction for all xCovariate
#' 
...
plot.fit = function(input, newFunc, 
  ...
  ...)
{
  func = if(missing(newFunc)) input$func else newFunc
  ...
  plotData = input$pred$pred

  ## labels
  if(missing(yLabel)) yLabel = input$method
  ...
  myTitle = if(missing(title))
    paste("predict(", input$object.name,", level = ", if(is.null(input$level)) 1 else input$level, ")", sep = "") else title
  ...
  thePlot = xyplot(plotForm,
      ...
      #subscripts = pData$dose==10
    {
      ...
      if(mnplot)
      {
        ...
        xpar = transform.log10(xpar, lgx)
        ypar = transform.log10(ypar, lgy)
        ylo  = transform.log10(ylo, lgy)
        yup  = transform.log10(yup, lgy)        
      ...
      if(!mnplot)
      {
         ...
         obsData[mySubs, "partres"] = transform.log10(obsData[mySubs, "partres"], lgx)
         obsData$partres[mySubs] = transform.log10(obsData$partres[mySubs], lgy)
