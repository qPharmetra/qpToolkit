...
#' @param special.x list of length 2, with first (numeric) element being the number that will be exchanged for the 2nd element which should be a text string. 
#' @return A formatted character vector containing count and percentage of 
#' unique values in \code{y}.
#' @export orderedNumeric
#' @import Hmisc
#' @examples
#' out = expand.grid(ID=1:5, DOSE = c(0,100,500,100,4000))
#' factor(out$DOSE)
